...
type ChannelProbeFrequency string

const (
	ChannelProbeFrequency1Min  ChannelProbeFrequency = "1m"
	ChannelProbeFrequency5Min  ChannelProbeFrequency = "5m"
	ChannelProbeFrequency30Min ChannelProbeFrequency = "30m"
	ChannelProbeFrequency1Hour ChannelProbeFrequency = "1h"
)

type ChannelProbeIntervalMode string

const (
	ChannelProbeIntervalModeFixed  ChannelProbeIntervalMode = "fixed"
	ChannelProbeIntervalModeRandom ChannelProbeIntervalMode = "random"
)
...
type ChannelSettings struct {
	...
	// ProbeFrequency overrides the system-level active health probe frequency for this channel.
	// Allowed values: "1m", "5m", "30m", "1h". Empty means using the system default.
	ProbeFrequency ChannelProbeFrequency `json:"probeFrequency,omitempty"`

	// ProbeEnabled controls whether this channel participates in auto health probing.
	// When false, this channel will be skipped even if system-level probe is enabled.
	// When nil/missing, defaults to true (channel participates in probing).
	ProbeEnabled *bool `json:"probeEnabled,omitempty"`

	// ProbeIntervalMode controls how the channel probe interval is scheduled.
	// Empty means falling back to legacy probeFrequency or the system default.
	ProbeIntervalMode ChannelProbeIntervalMode `json:"probeIntervalMode,omitempty"`

	// ProbeFixedIntervalSeconds is the fixed probe interval in seconds.
	ProbeFixedIntervalSeconds int `json:"probeFixedIntervalSeconds,omitempty"`

	// ProbeRandomMinIntervalSeconds is the inclusive minimum random interval in seconds.
	ProbeRandomMinIntervalSeconds int `json:"probeRandomMinIntervalSeconds,omitempty"`

	// ProbeRandomMaxIntervalSeconds is the inclusive maximum random interval in seconds.
	ProbeRandomMaxIntervalSeconds int `json:"probeRandomMaxIntervalSeconds,omitempty"`
}
