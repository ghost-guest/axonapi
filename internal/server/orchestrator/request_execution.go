...
func (m *persistRequestExecutionMiddleware) OnOutboundLlmResponse(ctx context.Context, llmResp *llm.Response) (*llm.Response, error) {
	...
	if channel != nil {
		candidate := state.ChannelModelsCandidates[state.CurrentCandidateIndex]
		entry := candidate.Models[state.CurrentModelIndex]
		if binder, ok := state.RetryPolicyProvider.(interface {
			BindPublicBenefitSessionAffinity(ctx context.Context, apiFormat llm.APIFormat, requestedModel, baseURL string)
		}); ok {
			requestedModel := entry.RequestModel
			if requestedModel == "" {
				requestedModel = state.OriginalModel
			}
			binder.BindPublicBenefitSessionAffinity(ctx, m.outbound.APIFormat(), requestedModel, channel.BaseURL)
		}
	}
