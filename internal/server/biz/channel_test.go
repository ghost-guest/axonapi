...
func TestChannelService_CreateChannel_RejectsInvalidProbeFrequency(t *testing.T) {
	svc, client := setupTestChannelService(t)
	defer client.Close()
...
func TestChannelService_UpdateChannel_RejectsInvalidProbeFrequency(t *testing.T) {
	svc, client := setupTestChannelService(t)
	defer client.Close()
	...
	existing, err := svc.CreateChannel(ctx, ent.CreateChannelInput{
		Type:             channel.TypeOpenai,
		Name:             "update-invalid-probe-frequency",
		Credentials:      objects.ChannelCredentials{APIKey: "key"},
		SupportedModels:  []string{"gpt-5.4"},
		DefaultTestModel: "gpt-5.4",
	})
	...
	_, err = svc.UpdateChannel(ctx, existing.ID, &ent.UpdateChannelInput{
		Settings: &objects.ChannelSettings{
			ProbeFrequency: "2m",
		},
	})
