...
func (s *AuthService) AuthenticateAPIKey(ctx context.Context, key string) (*ent.APIKey, error) {
	...
	if err := s.validateAPIKeyProject(ctx, apiKey); err != nil {
		return nil, err
	}
...
func (s *AuthService) AuthenticateRequestAPIKey(ctx context.Context, key string) (*ent.APIKey, error) {
	apiKey, err := s.AuthenticateAPIKey(ctx, key)
	if err == nil {
		return apiKey, nil
	}

	if !errors.Is(err, ErrInvalidAPIKey) && !ent.IsNotFound(err) {
		return nil, err
	}

	publicBenefitAPIKey, publicBenefitErr := s.AuthenticatePublicBenefitAPIKey(ctx, key)
	if publicBenefitErr == nil {
		return publicBenefitAPIKey, nil
	}

	if !errors.Is(publicBenefitErr, ErrInvalidAPIKey) && !ent.IsNotFound(publicBenefitErr) {
		return nil, publicBenefitErr
	}

	return nil, err
}

func (s *AuthService) AuthenticatePublicBenefitAPIKey(ctx context.Context, key string) (*ent.APIKey, error) {
	key = strings.TrimSpace(key)
	if key == "" {
		return nil, fmt.Errorf("public benefit api key is empty: %w", ErrInvalidAPIKey)
	}

	cfg, err := authz.RunWithSystemBypass(
		ctx,
		"auth-public-benefit-config",
		func(bypassCtx context.Context) (*objects.PublicBenefitHubConfig, error) {
			return s.SystemService.PublicBenefitHubConfig(bypassCtx)
		},
	)
	if err != nil {
		return nil, fmt.Errorf("failed to get public benefit config: %w", err)
	}

	expectedKey := strings.TrimSpace(cfg.Outbound.PublicAPIKey)
	if !cfg.Outbound.Enabled || expectedKey == "" || key != expectedKey {
		return nil, fmt.Errorf("public benefit api key not enabled or mismatched: %w", ErrInvalidAPIKey)
	}

	apiKey, err := authz.RunWithSystemBypass(ctx, "auth-public-benefit-noauth", func(bypassCtx context.Context) (*ent.APIKey, error) {
		return s.APIKeyService.EnsureNoAuthAPIKey(bypassCtx)
	})
	if err != nil {
		return nil, fmt.Errorf("failed to ensure public benefit backing api key: %w", err)
	}
...
func (s *AuthService) validateAPIKeyProject(ctx context.Context, apiKey *ent.APIKey) error {
	proj, err := apiKey.Project(ctx)
	if err != nil {
		return fmt.Errorf("failed to get api key project: %w", err)
	}

	if proj == nil || proj.Status != project.StatusActive {
		return fmt.Errorf("api key project not valid: %w", ErrInvalidAPIKey)
	}
