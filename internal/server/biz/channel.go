...
func validateChannelProbeFrequency(frequency objects.ChannelProbeFrequency) error {
	switch frequency {
	case "",
		objects.ChannelProbeFrequency1Min,
		objects.ChannelProbeFrequency5Min,
		objects.ChannelProbeFrequency30Min,
		objects.ChannelProbeFrequency1Hour:
		return nil
	default:
		return fmt.Errorf("invalid probe frequency %q, allowed values are 1m, 5m, 30m, 1h", frequency)
	}
}
...
func (svc *ChannelService) createChannel(ctx context.Context, input ent.CreateChannelInput) (*ent.Channel, error) {
	if err := validateChannelSettings(input.Settings); err != nil {
		return nil, err
	}
