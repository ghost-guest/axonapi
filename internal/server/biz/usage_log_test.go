...
func TestUsageLogService_CreateUsageLog_RecordsPublicBenefitUsage(t *testing.T) {
	client := enttest.NewEntClient(t, "sqlite3", "file:ent?mode=memory&_fk=0")
	defer client.Close()
	...
	ch, err := client.Channel.Create().
		SetName("public-benefit/u1").
		SetType(channel.TypeOpenaiResponses).
		SetBaseURL("https://relay.example/v1").
	...
	require.Equal(t, "u1", state.Upstreams[0].UpstreamID)
	require.EqualValues(t, 1, state.Upstreams[0].TotalRequests)
	...
	require.EqualValues(t, 100, state.DailyUsage[0].TotalTokens)
