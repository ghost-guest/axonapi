...
func (s *UsageLogService) CreateUsageLog(ctx context.Context, params CreateUsageLogParams) (*ent.UsageLog, error) {
	...
	if ch := s.ChannelService.GetEnabledChannel(params.ChannelID); ch != nil {
		if err := s.SystemService.RecordPublicBenefitUsage(ctx, ch.Name, params.Usage.TotalTokens); err != nil {
			log.Warn(ctx, "failed to record public benefit usage", log.Cause(err), log.String("channel_name", ch.Name))
		}
	}
