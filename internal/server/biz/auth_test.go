...
func TestAuthService_AuthenticateRequestAPIKey_PublicBenefit(t *testing.T) {
	authService, client, cleanup := setupTestAuthService(t, xcache.Config{})
	defer cleanup()
	defer client.Close()
	...
	owner, err := client.User.Create().
		SetEmail(fmt.Sprintf("owner-public-benefit-%d@example.com", time.Now().UnixNano())).
		SetPassword(hashedPassword).
		SetFirstName("Owner").
		SetLastName("User").
		SetIsOwner(true).
	...
	err = authService.SystemService.SetPublicBenefitHubConfig(ctx, objects.PublicBenefitHubConfig{
		Outbound: objects.PublicBenefitOutboundConfig{
			Enabled:       true,
			PublicAPIKey:  "pb-unified-key",
			PublicBaseURL: "https://hub.example",
		},
	})
	require.NoError(t, err)

	authenticatedAPIKey, err := authService.AuthenticateRequestAPIKey(ctx, "pb-unified-key")
	require.NoError(t, err)
	require.Equal(t, NoAuthAPIKeyValue, authenticatedAPIKey.Key)
	require.Equal(t, apikey.TypeNoauth, authenticatedAPIKey.Type)
	require.NotNil(t, authenticatedAPIKey.Edges.Project)
	require.Equal(t, defaultProject.ID, authenticatedAPIKey.Edges.Project.ID)

	createdNoAuthKey, err := client.APIKey.Query().Where(apikey.KeyEQ(NoAuthAPIKeyValue)).Only(ctx)
	require.NoError(t, err)
	require.Equal(t, owner.ID, createdNoAuthKey.UserID)
	require.Equal(t, defaultProject.ID, createdNoAuthKey.ProjectID)
}

func TestAuthService_AuthenticateRequestAPIKey_PublicBenefitDisabled(t *testing.T) {
	authService, client, cleanup := setupTestAuthService(t, xcache.Config{})
	defer cleanup()
	defer client.Close()
	...
	err := authService.SystemService.SetPublicBenefitHubConfig(ctx, objects.PublicBenefitHubConfig{
		Outbound: objects.PublicBenefitOutboundConfig{
			Enabled:      false,
			PublicAPIKey: "pb-disabled-key",
		},
	})
	require.NoError(t, err)

	_, err = authService.AuthenticateRequestAPIKey(ctx, "pb-disabled-key")
	require.Error(t, err)
